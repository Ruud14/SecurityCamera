...
class Authentication:
    def __init__(self, ip):
        ...
        self.url = "http://{}:8000/stream.mjpg".format(ip)
    ...
    def authenticate(self, username, password):
        try:
            ...
            print("Camera login failed. You are probably using the wrong credentials, or the camera is not running.")
            return False
...
class Detector:
    def __init__(self, ip, username, password, sensitivity, detection_resolution=(80, 45)):
        ...
        self.cap = cv2.VideoCapture('http://{}:{}@localhost:8000/stream.mjpg'.format(*self.credentials))
    ...
    # Returns a resized version of the last frame from the live camera feed.
    def _get_current_low_res_frame(self):
    ...
    def _detect_motion(self):
        ...
        start_frame = cv2.cvtColor(self._get_current_low_res_frame(), cv2.COLOR_BGR2GRAY)
        start_frame = cv2.GaussianBlur(start_frame, blur, 0)
        while True:
            # get the current frame.
            next_frame = cv2.cvtColor(self._get_current_low_res_frame(), cv2.COLOR_BGR2GRAY)
